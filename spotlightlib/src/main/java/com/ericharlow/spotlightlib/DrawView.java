...
/**
 * This view will draw all {@link SpotlightPoint} on its surface. It uses a {@link DrawViewAdapter } to get the content to
 * draw.
 *
 * @author sni
 */
public class DrawView extends View {
     ...
     */
    public void setDrawViewAdapterSpotlightPoints(List< SpotlightPoint > listPoints) {
    ...
    /**
     * Draw some surface under text. This method is called just before drawing each {@link SpotlightPoint}'s text.
     *
     * @param canvas
     *            the canvas in which we draw.
     * @param layout
     *            the {@link Layout} associated to a {@link SpotlightPoint}.
     */
     ...
     * associated to a {@link SpotlightPoint}. This method is called just before drawing each {@link SpotlightPoint}'s
     * drawable.
     *
     * @param canvas
     *            the canvas on which to draw the point at position.
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (isTouchHandler() && isTouchEventInSpotlightPoint(event, drawViewAdapter.getDrawableAt(currentPointPositionToDisplay))) {
            if (event.getAction() == MotionEvent.ACTION_UP)
                showNextPoint();
            if (dispatchTouchHandler != null)
                return dispatchTouchHandler.sendTouchEvent(event);
            else
                return true;
        }
     ...
     * @param - position of the {@link SpotlightPoint}.
     */
    ...
    private boolean isTouchEventInSpotlightPoint(MotionEvent event, Drawable drawable) {
        float x = event.getX();
        float y = event.getY();
    ...
    public DispatchTouchHandler getDispatchTouchHandler() {
        return dispatchTouchHandler;
    }

    public void setDispatchTouchHandler(DispatchTouchHandler dispatchTouchHandler) {
